    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:  # agar hands detect hue to
            for landmarks in results.multi_hand_landmarks:
                ...
                if handedness == "Right" and not switching_tabs and index_tip.y < index_mid.y:  # right hand and index finger raised
                    threading.Thread(target=switch_tab, args=("right",)).start()

                if handedness == "Left" and not switching_tabs and index_tip.y < index_mid.y:  # left hand and index finger raised
